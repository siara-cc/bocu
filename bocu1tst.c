...
/**
 * Converts from UTF-8 bytes to BOCU-1 bytes.
 * Also outputs interesting statistics.
 ...
 */
static unsigned long
encodeString(uint8_t *in, uint8_t *out) {
    uint8_t bytes[4];
    unsigned long inLength, inCount, outLength;
    ...
    prev=0;
    i=read=0;
    count = read = inLength = strlen(in);

    /* get code points from UTF-8 and write in BOCU-1 */
    for(i=0; i<count;) {
        UTF8_NEXT_CHAR_SAFE(in, i, read, c, FALSE);
...
main(int argc, const char *argv[]) {
    ...
    if(argc>1) {
        ...
        if( strcmp(argv[1], "?")==0 || strcmp(argv[1], "-?")==0 ||
            ...
            printf("Converting \"%s\"...\n", argv[2]);
            unsigned long enc_len = encodeString((uint8_t *) argv[2], enc_buf);
            decodeString(enc_buf, enc_len, out_buf);
